...
- Path to profiler/eval (`make profile-nat`, `make eval`) without changing the Pipefy integration.
...
Only **`inventory`** is wired into `make tour`. Scenarios B and C run via `make demo-*` and are scored by `make eval` — see [BENCHMARKS.md](BENCHMARKS.md).
...
After you can run **Step 2** locally (`make demo-cursor`), you can run the **same `inventory` prompt** in a **Cursor-hosted VM** instead of on your laptop:

```bash
make demo-cloud SCENARIO=inventory
```

Prerequisites (GitHub SCM in [cursor.com/agents](https://cursor.com/agents), `.env` secrets, pushed bootstrap on your target branch) and full runbook: **[CLOUD.md](CLOUD.md)**.

Cloud is a **secondary** path—the numbered Steps 0–4 above stay the primary hands-on tour.
