...
| D18 | NIM `70b` API tier check | **8b measured; 70b not measured in PRD-2 close-out.** MVP / golden eval uses **8b** only (`meta/llama-3.1-8b-instruct`). **PRD-2 (2026-06-03):** `make eval` on `inventory` — NAT **0/3** first-attempt pass, **0/3** with-retries ([BENCHMARKS.md](BENCHMARKS.md)). Operators may set `70b` via workflow YAML or `EVAL_ARGS=--model meta/llama-3.1-70b-instruct` if [NVIDIA Build](https://build.nvidia.com/) tier allows; no automated check in `make doctor`. Prior ad-hoc 70b probe (2025-06-02) still in [LEARNINGS.md](LEARNINGS.md). | 2026-06-03 |
| D19 | NAT tour reliability contract | **Act 2 (Cursor) = guaranteed-green in tour; Act 3 (NAT 8b) = best-effort** (compare-gated retries). **PRD-2 measured (2026-06-03):** golden `inventory` eval — NAT 8b **0%** first-attempt (**0/3**), **0%** with-retries (**0/3**), median **19.86 s** / p90 **31.27 s**; Cursor smoke **0/1** on same date ([BENCHMARKS.md](BENCHMARKS.md)). Confirms D19: do not treat NAT 8b as guaranteed-green on golden facts; keep 8b default (D9/D18). Tour hardening ~3/5 first-attempt remains informal; see [LEARNINGS.md](LEARNINGS.md). | 2026-06-03 |
