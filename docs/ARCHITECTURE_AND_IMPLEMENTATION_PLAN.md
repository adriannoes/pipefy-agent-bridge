# Pipefy Agent Bridge — Architecture

**Repository:** `pipefy-agent-bridge`  
**Version:** 1.0  
**Audience:** Developers and operators evaluating or extending the bridge  
...
pipefy-mcp-server registers **149 tools across ten domains** (confirm on the pinned version). NAT workflows should pass `include:` with **8–15 tools** for the core tour (inventory + read + one safe write).
...
**Suggested core-tour tool names** (verify against `PIPEFY_TOOL_NAMES` in `pipefy/ai-toolkit` → `packages/mcp/src/pipefy_mcp/tools/registry.py`):
...
> **Shipped:** The allowlist is frozen in `configs/tool_allowlist.yml` (12 tools @ pinned toolkit tag).
...
**Cloud agents:** Shipped optional path (`CloudAgentOptions` + `repos=[...]`) to run Pipefy automation in Cursor Cloud VMs. Requires a GitHub-hosted repo and SCM integration — see [`docs/CLOUD.md`](CLOUD.md) and `make demo-cloud`.
...
**Serving (optional):** `nat mcp serve` exposes a workflow as MCP—useful if you later want Cursor to call a NAT-wrapped workflow. **Not required** for the core tour; see [Future work](#20-future-work) (D17).
...
Eval should compare **structured facts** (counts, IDs, phase names) extracted from CLI JSON vs agent answer (manual rubric or light LLM-judge—keep the first version simple).
...
**Cursor cloud `env_vars`:** Cloud agents use `CloudAgentOptions.env_vars` for session-scoped secrets (encrypted at rest per Cursor docs)—see [`docs/CLOUD.md`](CLOUD.md).
...
| **NIM** | LLM via `_type: nim` and `NVIDIA_API_KEY`; embeddings in Phase 7 |
| **MCP integration** | `nvidia-nat[mcp]` package |

### 11.2 Out of scope (by design)
...
| **Full RAG platform** | Out of scope; Phase 7 covers bounded semantic search only |

### 11.3 GPU embeddings (Phase 7 — shipped)
...
## 12. Optional phase: GPU embeddings and semantic search — **shipped** (2026-06-04)

### 12.1 User-facing capability

> “Find cards semantically similar to this bug report” across a demo pipe with hundreds of cards.

**Shipped:** `make similar QUERY="..."` over `DEMO_PIPE_ID` (bounded card set); see [`embeddings/README.md`](../embeddings/README.md).

### 12.2 Architecture add-on

```
Pipefy cards (CLI) → text fields → NIM or local embeddings → FAISS index (CPU default) → top-k results
```

GPU index backend is opt-in (`EMBED_BACKEND=gpu`); embeddings default to NIM **`nvidia/nv-embedqa-e5-v5`** with local sentence-transformers fallback.

### 12.3 Dependencies

- `NVIDIA_API_KEY` with embedding model access (or `EMBED_PROVIDER=local`)
- Optional extra: `uv sync --extra embeddings` (`faiss-cpu`, `numpy`, `sentence-transformers`)
- GPU index: `faiss-gpu` / cuVS only when operator sets `EMBED_BACKEND=gpu`

### 12.4 Prerequisites

**Satisfied** — core Cursor + NAT demos were green before the embeddings module. Phase 7 lives under `embeddings/` (D16 resolved in [OPEN_DECISIONS.md](OPEN_DECISIONS.md)).
...
│   ├── 03_cursor_cloud_ops.py
│   └── prompts/
│       ├── inventory.txt
│       ├── stale_cards.txt
│       └── summary.txt
├── embeddings/                 # semantic search (Phase 7)
│   └── README.md
├── eval/
│   ├── ground_truth.sh         # pipefy CLI JSON snapshots
│   ├── compare.py              # lightweight fact checker
│   ├── golden.yaml             # golden eval scenarios
│   ├── run_eval.py             # reliability runner (make eval)
...
│   ├── BENCHMARKS.md           # reliability & latency report
│   ├── CLOUD.md                # Cursor Cloud operator guide
│   └── OPEN_DECISIONS.md       # resolved and open decisions
```

**Justification:** Separating `demos/`, `configs/`, and `eval/` mirrors the dual-harness split (Cursor / NVIDIA / validation).

---

## 14. Delivery phases

> **Core tour (Phases 0–4 shipped):** Upstream pinned at **`pipefy/ai-toolkit@v0.2.0-beta.2`** (`PIPEFY_TOOLKIT_REF` in `Makefile`; mirrored in `configs/tool_allowlist.yml` as `pipefy_toolkit_ref`). **`configs/tool_allowlist.yml`** curates **12** MCP tool names (verified with `nat mcp client tool list --transport stdio --command pipefy-mcp-server` for that tag). NAT workflow uses **`meta/llama-3.1-8b-instruct`** on NIM and an **inventory-only 2-tool** `include:` (`search_pipes`, `get_cards`) in `configs/pipefy_nat_workflow.yml` (§16.5). Phases 5–7 shipped (golden eval & benchmarks, Cursor Cloud, GPU semantic search).
...
**Exit criteria:** All Phase 0 items complete; `make help` lists operator targets.
...
### Phase 5 — NVIDIA profiler & eval — **shipped**
...
### Phase 6 — Cursor Cloud agent — **shipped**

**Status:** Done — `make demo-cloud`, [`docs/CLOUD.md`](CLOUD.md), `.cursor/environment.json` + `scripts/cloud_bootstrap.sh` (2026-06-04).

Tasks:

1. [x] Push repo to GitHub and connect SCM in Cursor ([`docs/CLOUD.md`](CLOUD.md)).
2. [x] Cloud agent with `CloudAgentOptions` + `repos=[...]` + MCP stdio in VM (`demos/03_cursor_cloud_ops.py`, `demos/_cloud_config.py`, `make demo-cloud`).
3. [x] Document `skip_reviewer_request` for quiet CI ([`docs/CLOUD.md`](CLOUD.md); `build_cloud_options` default `skip_reviewer_request=True`).

**Exit criteria:** [x] A cloud agent run completes the `inventory` scenario against Pipefy MCP in a Cursor-hosted VM; `agent_id` (`bc-…`) is logged.

### Phase 7 — GPU semantic search — **shipped**

**Status:** Done — `embeddings/` module, `make similar`, [`embeddings/README.md`](../embeddings/README.md) (2026-06-04). See Section 12.

Tasks:

1. [x] NIM embedding model + tier note documented in `embeddings/README.md`.
2. [x] Bounded card text via Pipefy CLI (`embeddings/card_text.py`, cap 100).
3. [x] `embed_texts()` + pluggable FAISS index — CPU default, `EMBED_BACKEND=gpu` opt-in (D16).
4. [x] `make similar QUERY="..."` + offline ranking tests (`embeddings/rank.py`, `tests/test_rank.py`).

**Exit criteria:** [x] "Find similar cards" returns top-k over the demo pipe via NIM (or local) embeddings + vector index, with documented CPU fallback and GPU opt-in.
...
### 17.2 Core tour eval (shipped — Phase 4)
...
Shipped artifacts (Phase 5):

1. **Golden set** — `eval/golden.yaml` + `eval/golden_loader.py`; `inventory`, `stale_cards`, and `summary` scenarios.
...
- **D17:** Publish the NAT workflow as an MCP server via `nat mcp serve` (second harness exposure).
- **Pipefy toolkit pagination:** Reduce N× `get_cards` for large org inventories when upstream adds helpers.
- **NAT 8b first-attempt reliability:** Improve without relying solely on tour retries; `70b` remains the documented upgrade path (D18).
...
- [Cloud Agents API](https://cursor.com/docs/cloud-agent/api/endpoints) — see [`docs/CLOUD.md`](CLOUD.md)
...
| 0.2 | 2026-06-02 | Reproducible tour (TRY_IT_YOURSELF); docs index; OPEN_DECISIONS |
| 0.3 | 2026-06-02 | Consistency audit; docs index; OPEN_DECISIONS; Makefile naming aligned |
...
| 0.6 | 2026-06-02 | Core tour shipped: §2.3 and §14 Phases 0–4 marked complete; pinned `v0.2.0-beta.2`, 12-tool allowlist, NIM `meta/llama-3.1-8b-instruct` recorded |
| 1.0 | 2026-06-04 | Public-docs pass: removed internal planning references; Phases 5–7 marked shipped; §20 replaced with future work |
