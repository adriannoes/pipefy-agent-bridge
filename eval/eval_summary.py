"""Aggregate evaluation run results into reliability and latency summaries."""
...
class HarnessSummary:
    """Aggregated metrics for one scenario and harness."""
...
def summarize(results: list[RunResult], retries: int) -> Summary:
    ...
    harnesses = _scenario_harnesses_in_results(results)
    summaries: list[HarnessSummary] = []
    for scenario, harness in harnesses:
        harness_rows = [
            row for row in results if row.scenario == scenario and row.harness == harness
        ]
...
def _scenario_harnesses_in_results(
    results: list[RunResult],
) -> tuple[tuple[str, HarnessName], ...]:
    """Stable (scenario, harness) pairs in first-seen order."""
    seen: set[tuple[str, HarnessName]] = set()
    ordered: list[tuple[str, HarnessName]] = []
    for row in results:
        key = (row.scenario, row.harness)
        if key in seen:
            continue
        seen.add(key)
        ordered.append(key)
...
def format_benchmark_markdown_table(summary: Summary) -> str:
    ...
    header = (
        "| Scenario | Harness | N | First-attempt % | With-retries % | "
        "Median latency (s) | P90 latency (s) |"
    )
    separator = "| --- | --- | ---: | ---: | ---: | ---: | ---: |"
    rows = [
        (
            f"| {row.scenario} | {row.harness} | {row.n_episodes} | "
