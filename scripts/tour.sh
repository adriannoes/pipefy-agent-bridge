#!/bin/sh
# Full operator tour: Acts 1–3 + eval fact check (ARCHITECTURE §16.5).
...
run_demo_cursor_once() {
	PYTHONPATH="$REPO_ROOT" uv run python "$REPO_ROOT/demos/01_cursor_pipefy_ops.py" --scenario "$TOUR_SCENARIO" \
		>"$CURSOR_ANSWER_PATH"
}
