...
from eval.compare import (
    ...
    compare_stale_cards_facts,
    compare_summary_facts,
    evaluate_inventory_answer,
    evaluate_stale_cards_answer,
    evaluate_summary_answer,
    extract_answer_facts,
    extract_baseline_facts,
    extract_stale_cards_answer_facts,
    extract_stale_cards_baseline_facts,
    extract_summary_answer_facts,
...
EXAMPLE_STALE_CARDS_PATH = Path("eval/fixtures/example/stale_cards.json")
EXAMPLE_SUMMARY_PATH = Path("eval/fixtures/example/summary.json")
...
@pytest.fixture
def example_stale_cards_baseline() -> dict[str, object]:
    return json.loads(EXAMPLE_STALE_CARDS_PATH.read_text(encoding="utf-8"))


@pytest.fixture
def example_summary_baseline() -> dict[str, object]:
    return json.loads(EXAMPLE_SUMMARY_PATH.read_text(encoding="utf-8"))


def test_extract_stale_cards_baseline_facts(
    example_stale_cards_baseline: dict[str, object],
) -> None:
    facts = extract_stale_cards_baseline_facts(example_stale_cards_baseline)
    assert facts.stale_count == 2
    assert facts.card_titles == ("Old onboarding request", "Stale helpdesk ticket")


def test_stale_cards_exact_match_passes(example_stale_cards_baseline: dict[str, object]) -> None:
    answer = (
        "Found 2 stale cards in Waiting Review:\n"
        "- Old onboarding request\n"
        "- Stale helpdesk ticket\n"
    )
    assert evaluate_stale_cards_answer(example_stale_cards_baseline, answer) is True


def test_stale_cards_missing_title_fails(example_stale_cards_baseline: dict[str, object]) -> None:
    answer = "Found 2 stale cards: Old onboarding request only."
    assert evaluate_stale_cards_answer(example_stale_cards_baseline, answer) is False


def test_stale_cards_count_mismatch_fails(example_stale_cards_baseline: dict[str, object]) -> None:
    answer = "Found 5 stale cards:\n- Old onboarding request\n- Stale helpdesk ticket\n"
    assert evaluate_stale_cards_answer(example_stale_cards_baseline, answer) is False


def test_extract_summary_baseline_facts(example_summary_baseline: dict[str, object]) -> None:
    facts = extract_summary_baseline_facts(example_summary_baseline)
    assert facts.card_count == 3
    assert facts.card_titles == (
        "Newest open request",
        "Second newest card",
        "Third backlog item",
    )
...
def test_summary_exact_match_passes(example_summary_baseline: dict[str, object]) -> None:
    answer = (
        "Top 3 newest open cards:\n"
        "1. Newest open request (In Progress)\n"
        "2. Second newest card (Submitted)\n"
        "3. Third backlog item (Submitted)\n"
    )
    assert evaluate_summary_answer(example_summary_baseline, answer) is True


def test_summary_missing_title_fails(example_summary_baseline: dict[str, object]) -> None:
    answer = "3 cards: Newest open request and Second newest card."
    assert evaluate_summary_answer(example_summary_baseline, answer) is False


def test_compare_stale_cards_facts_requires_full_title_set() -> None:
    baseline = extract_stale_cards_baseline_facts(
        {
            "scenario": "stale_cards",
            "stale_cards": [{"id": "1", "title": "Alpha"}, {"id": "2", "title": "Beta"}],
            "stale_count": 2,
        }
    )
    partial_answer = extract_stale_cards_answer_facts(
        "2 stale cards: Alpha",
        baseline.card_titles,
        baseline.card_ids,
    )
    assert compare_stale_cards_facts(baseline, partial_answer) is False


def test_compare_summary_facts_requires_full_title_set() -> None:
    baseline = extract_summary_baseline_facts(
        {
            "scenario": "summary",
    ...
    partial_answer = extract_summary_answer_facts("2 cards: Alpha", baseline.card_titles)
    assert compare_summary_facts(baseline, partial_answer) is False
