...
def test_load_golden_inventory_case() -> None:
    ...
    stale = next(case for case in cases if case.scenario == "stale_cards")
    assert stale.expect["min_stale_cards"] == 1

    summary = next(case for case in cases if case.scenario == "summary")
    assert summary.expect["min_cards"] == 3
